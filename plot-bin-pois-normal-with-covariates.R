...
parameter_grid_pois <- expand.grid(
   # n = c(50,seq.int(from = 100, to = 500, by = 100))    # sample size
   n = seq.int(from = 50, to = 500, by = 50)    # sample size
...
# bin_pois_normal_B_500000_results <- readRDS("~/R/R Codes/Experiments with R/temp-spacrt-result/bin_pois_normal_B_500000_results.rds")


for(k in 2:5){
   plot.temp <- bin_pois_normal_B_500000_results$metrics[1:120, ] |>
      dplyr::filter(n != 50) |>
   ...
   assign(paste0('plot.pois.toe.',k), plot.temp)
   print(k)
}


ggpubr::ggarrange(plot.pois.toe.2,
                  plot.pois.toe.3,
                  plot.pois.toe.4,
                  plot.pois.toe.5,
                  common.legend = T,
...
# ggsave('plot-bin-pois-normal-with-covariates-toe.pdf')

df.pvals <- df.qq <- df.qq.proper <- qq_plots_pois <- list()

for(i in sort(unique(bin_pois_normal_B_500000_results$results$grid_id))){
   
   df.qq[[i]] <- bin_pois_normal_B_500000_results$results %>%
      # dplyr::select(run_id) %>%
   ...
   (qq_plots_pois[[i]] <- df.qq.proper[[i]] %>%
      ...
      theme_light() +
      ggtitle(paste0("n = ",parameter_grid_pois$n[i])) +
      theme(legend.position = "bottom", 
