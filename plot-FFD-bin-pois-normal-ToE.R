...
#####################
results_rds <- readRDS("~/R/R Codes/Experiments with R/temp-spacrt-result/grid_explore_FFD_bin_pois_normal_B_5000_5e3_n5_n5_w_score_results.rds")
#####################
...
for(toe in unique(results_rds_metrics$metric)){
   ...
   page.heading[which(unique.metrics == toe)] <- 
      paste0("X|Z ~ Bernoulli, ",
             "Y|Z ~ Poisson : Level = ",level,"\n")
}

#################

ggsave(filename = "plot-FFD-bin-pois-normal-5000-5e3-n5-n5-w-score-ToE.pdf", 
